...
function UserProfileContent() {
  ...
  if (loading) {
    return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0a0a0a", color: "#fff" }}>Loading profile...</div>;
  }
  if (error || !profile) {
    return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0a0a0a", color: "#ff4444" }}>{error || "Profile not found"}</div>;
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", padding: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ maxWidth: 900, margin: "40px auto 0 auto", background: "#000000", borderRadius: 28, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", overflow: "hidden", position: "relative", paddingBottom: 24, border: "2px solid #4FC3F7" }}>
        {/* Back button */}
        <button onClick={() => router.back()} style={{ position: "absolute", top: 18, left: 18, background: "#000000", border: "2px solid #4FC3F7", borderRadius: 10, fontSize: 18, padding: "4px 14px", cursor: "pointer", zIndex: 10, boxShadow: '0 2px 8px rgba(79,195,247,0.3)', fontWeight: 700, color: '#4FC3F7', transition: 'all 0.2s' }}>←</button>
        {/* Profile Photo */}
        <div style={{ width: "100%", height: 220, background: "#1a1a1a", position: "relative" }}>
        ...
        {morePhotos.length > 0 && (
          <div style={{ padding: "16px 24px 0 24px" }}>
            <div style={{ fontWeight: 700, color: "#4FC3F7", fontSize: 16, marginBottom: 8 }}>More Photos</div>
            <div style={{ display: "flex", gap: 12, overflowX: "auto" }}>
              {morePhotos.map((url, idx) => (
                <img
                  ...
                  style={{ width: 80, height: 80, objectFit: "cover", borderRadius: 12, border: "2px solid #4FC3F7", flexShrink: 0, boxShadow: '0 2px 8px rgba(79,195,247,0.3)' }}
                />
              ))}
            </div>
          </div>
        ...
        <div style={{ padding: "24px 32px 0 32px" }}>
          <div style={{ fontSize: 32, fontWeight: 800, color: "#ffffff", marginBottom: 4, letterSpacing: 0.5 }}>
            {profile.name}
            {profile.birthdate && (
              <span style={{ fontWeight: 500, color: "#4FC3F7", fontSize: 22, marginLeft: 10 }}>
          ...
          {Array.isArray(profile.interests) && profile.interests.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontWeight: 700, color: "#4FC3F7", fontSize: 15, marginBottom: 7 }}>Interests</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {profile.interests.map((interest: string, idx: number) => (
                  <span key={idx} style={{ background: "linear-gradient(90deg, #4FC3F7 0%, #29B6F6 100%)", color: "#000000", borderRadius: 12, padding: "7px 16px", fontSize: 15, fontWeight: 600, letterSpacing: 0.2, boxShadow: '0 2px 8px rgba(79,195,247,0.3)' }}>{interest}</span>
                ))}
              </div>
            </div>
          )}
        ...
        {successMsg && (
          <div style={{ textAlign: 'center', color: '#4caf50', fontWeight: 700, fontSize: 16, marginTop: 18 }}>{successMsg}</div>
        )}
      </div>
